...
O desafio era capturar, estruturar, armazenar e transformar dados de Terceirizados de Órgãos Federais a partir do site https://www.gov.br/cgu/pt-br/acesso-a-informacao/dados-abertos/arquivos/terceirizados na AWS S3 e deixá-los prontos para consulta em uma API.

A arquitetura da solução utiliza:
- AWS S3 como Data Lake
- Prefect para orquestração
- DBT + DuckDB para transformação
- FastAPI para disponibilização dos dados
- Docker Compose para orquestração local

Abaixo, cada uma das etapas realizadas: 
...
Criação de uma API com FastAPI, rodando também em Docker, que consome os dados da camada gold criada no pipeline do Prefect e salva o arquivo `.duckdb` localmente para agilizar as consultas. A API expõe os seguintes entrypoints:
1. **GET `/terceirizados`**: consulta dos dados de todos os terceirizados retornando apenas algumas colunas. Possui paginação.

2. **GET `/terceirizados/{id}`**: consulta de um terceirizado específico a partir de seu id, retornando todas as colunas da tabela.

3.  **GET `/`**: encaminha para a página de documentação `/docs`.

3.  **POST `/terceirizados/admin/refresh`**: baixa novamente os dados da API a partir da camada gold do bucket S3. 

## Instruções para replicar localmente
### Pré requisitos
- Docker instalado no computador
- Conta ativa na AWS para criar o bucket S3 de armazenamento dos dados. É preciso também ter chaves de acesso criadas para conceder autorização aos containers para criar/acessar o bucket.

### Instruções
1. Clone este diretório no seu computador

2. Preencha o arquivo `.env_preencher` presente no diretório e depois o renomeie para `.env`. As variáveis de ambiente necessárias são as seguintes:
...
4. Aguarde até que todos os containers sejam criados e acesse os seguintes endereços no seu browser:
- **http://localhost:4200**: `Servidor Prefect` com o Flow já carregado e pronto para execução. Se o Flow não estiver aparecendo, aguarde alguns instantes e atualize a página.

- **http://localhost:8000**: `API para consulta dos dados`. Só vai funcionar depois de rodar o Flow do prefect ao menos uma vez.

- **http://localhost:4000**: `Documentação DBT` das tabelas criadas.